...
clean_type = ["aux", "nav", "snm", "blg", "bbl",
              "idx", "ind", "lof", "lot", "out",
              "toc", "acn", "acr", "alg", "glg",
              "glo", "gls", "ist", "fls", "log",
              "fdb_latexmk", "gz"]
...
sudo {2}'''.format(scriptName, packagesString, basicCommand)
    ...
    def getSearchResults(preamble, term, strictMatch):
        ...
        process = subprocess.Popen([tlmgr, "search", "--global", "--file", term], stdin=subprocess.PIPE,
    ...
    return (
    lambda entry: searchAndInstall(searchFilePackage, entry), lambda entry: searchAndInstall(searchFontPackage, entry))


def generateCompiler(compiler, arguments, texDoc, exiter):
    def compileTexDoc():
        try:
            process = subprocess.Popen([compiler] + shlex.split(arguments) + [texDoc], stdin=sys.stdin,
...
if __name__ == '__main__':
    ...
    for texDoc in texDocs:
        ...
        try:
            ...
            (installFile, installFont) = generateTLMGRFuncs(tlmgr_path, installSpeaker,
                                                            generateSudoer(options.terminal_only))
        except OSError:
